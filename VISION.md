...
*Written 2026-07-29, from the Syncr/agent-deck alignment discussion. Decisions referenced here
...
When a vision file and a Syncr issue disagree, the issue wins — then fix the vision file.*
...
prompts, tools, credentials) is simply *present* in the project, correctly scoped, without
...
1. **[VISION_EDGE.md](VISION_EDGE.md)** — *execution stays on the user's machine.* The cloud
   coordinates; it never executes. Agent Deck works fully offline; collaboration degrades,
   personal agent use never does.
2. **[VISION_WORKSPACE.md](VISION_WORKSPACE.md)** — *projects are the unit of collaboration.*
...
3. **[VISION_IDENTITY.md](VISION_IDENTITY.md)** — *every action chains to a human.* Owner-
...
what will change; parallel work stays organized; it feels native. The vision adds *who else is
in the room* — it never subtracts the calm, legible, single-user tool.
...
- Self-sovereign user keys as identity; any Nostr *dependency* (wire compatibility remains a
