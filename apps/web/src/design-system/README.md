...
- Sidebar-launched views compose `PageShell` + optional `PageToolbar` / `MasterDetailSplit` / `DetailHeader`. Use `width="page"` unless the view is a split/sticky editor (`split`). Hero title and canvas share `--space-page-x` so they line up; do not center a second column. `SectionHero` is 9rem (`--size-section-hero`). Settings rows use `AppSwitch`; inline status uses `AppInlineNotice`.
- Controls use three minimum-height tiers: compact 28px, standard 32px, and large 40px. Buttons, fields, selects, and segmented tracks use the shared 6px control radius (8px at the large button tier), semantic horizontal padding, and visible focus rings. Ordinary actions are not capsules; reserve the pill variant for chips, tags, and independent multi-select filters. `AppSegmentedPicker` uses `--radius-segmented` (6px), not a capsule. `PageToolbar` uses 28px controls, 8px in-cluster gap, 12px between clusters, and a 48px minimum row rather than clipping them.
- Switch ON fill is brand-action teal (`--color-switch-on` / `--color-primary`), never cyan accent. Cyan is identity, focus rings, and selection wash only.
