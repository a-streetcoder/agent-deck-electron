...
function parseWorktreePorcelainZ(output: string): PorcelainWorktree[] {
  ...
  for (const record of output.split("\0\0")) {
    ...
    entries.push({ path: worktree, ...(head ? { head } : {}), ...(branch ? { branch } : {}) });
  }
  return entries;
}

/**
 * Remove one proven Loop-owned worktree. Unlike gitWorktreeRemove, this helper
 * has no permissive recursive-rm fallback: every persisted identity, filesystem
 * boundary and live Git registration must agree before Git is allowed to remove
 * anything. A private 0700 managed parent plus the final no-follow lstat reduces
 * replacement races; standard path APIs cannot make this fully race-free.
 */
export async function strictRemoveOwnedLoopWorktree(options: {
  managedRoot: string;
  registeredProjectRoot: string;
  worktree: OwnedLoopWorktreeProof;
}): Promise<void> {
  const { worktree } = options;
  if (
    worktree.ownershipVersion !== 1 ||
    !/^[0-9a-f]{8}-[0-9a-f-]{27}$/i.test(worktree.ownershipId) ||
    worktree.branchOwned !== true ||
    !worktree.branch.startsWith("agent-deck/loop-")
  ) {
    throw new Error("invalid Loop worktree ownership proof");
  }

  const rootStat = lstatSync(options.managedRoot);
  if (!rootStat.isDirectory() || rootStat.isSymbolicLink()) {
    throw new Error("Loop managed root is not a private directory");
  }
  const canonicalRoot = realpathSync.native(options.managedRoot);
  if (normalizedNativePath(canonicalRoot) !== normalizedNativePath(options.managedRoot)) {
    throw new Error("Loop managed root is not canonical");
  }

  const registeredProject = realpathSync.native(options.registeredProjectRoot);
  const persistedProject = realpathSync.native(worktree.projectRoot);
  if (normalizedNativePath(registeredProject) !== normalizedNativePath(persistedProject)) {
    throw new Error("Loop worktree project ownership mismatch");
  }

  const expectedName = `loop-${worktree.ownershipId}`;
  if (
    path.basename(worktree.path) !== expectedName ||
    normalizedNativePath(path.dirname(worktree.path)) !== normalizedNativePath(canonicalRoot)
  ) {
    throw new Error("Loop worktree target is outside its managed root");
  }
  const targetStat = lstatSync(worktree.path);
  if (!targetStat.isDirectory() || targetStat.isSymbolicLink()) {
    throw new Error("Loop worktree target is not an owned directory");
  }
  const canonicalTarget = realpathSync.native(worktree.path);
  if (
    normalizedNativePath(canonicalTarget) !== normalizedNativePath(worktree.path) ||
    normalizedNativePath(path.dirname(canonicalTarget)) !== normalizedNativePath(canonicalRoot)
  ) {
    throw new Error("Loop worktree target is not canonical");
  }

  const entries = parseWorktreePorcelainZ(
    await runGit(registeredProject, ["worktree", "list", "--porcelain", "-z"]),
  );
  const entry = entries.find(
    (candidate) => normalizedNativePath(candidate.path) === normalizedNativePath(canonicalTarget),
  );
  if (!entry) throw new Error("Loop worktree is not registered with Git");
  if (entry.branch !== `refs/heads/${worktree.branch}` || !entry.head) {
    throw new Error("Loop worktree branch registration mismatch");
  }
  const branchHead = (
    await runGit(registeredProject, ["rev-parse", "--verify", `${worktree.branch}^{commit}`])
  ).trim();
  if (entry.head !== branchHead) throw new Error("Loop worktree HEAD ownership mismatch");

  // Git performs the authoritative removal. If it fails, do not recursively rm
  // a path that may have changed after verification.
  await runGit(registeredProject, ["worktree", "remove", "--force", canonicalTarget]);
  await runGit(registeredProject, ["worktree", "prune"]).catch(() => {});

  // Git can leave an empty directory behind on Windows. Recheck the direct child
  // without following a final symlink before the narrowly-scoped fallback rm.
  try {
    const remaining = lstatSync(worktree.path);
    if (
      ...
      throw new Error("refusing to remove a replaced Loop worktree target");
    }
    await rm(worktree.path, { recursive: true, force: false, maxRetries: 10, retryDelay: 300 });
  } catch (error) {
