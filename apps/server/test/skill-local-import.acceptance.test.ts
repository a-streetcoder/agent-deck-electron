...
describe.skipIf(!hasLocalFolder)("engine 0.1.7 local folder import routes (SKL-05/06)", () => {
  ...
  it("a skill with a binary asset previews and imports byte-identical (engine 0.2.0, Syncr #206)", async () => {
    ...
    // its own skill name: `alpha` is already in the catalog from the first test, and a second
    // import of the same name is (correctly) a 409 — that is not what this test is about
    ...
    writeFileSync(path.join(iconic, "SKILL.md"), "---\nname: Iconic\ndescription: Ships an icon\n---\nBody\n");
    writeFileSync(path.join(iconic, "reference.md"), "extra material\n");
    const png = Buffer.from([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a, 0x00, 0xff, 0xfe, 0x0d, 0x0a, 0x42]);
    writeFileSync(path.join(iconic, "assets", "icon.png"), png);

    const inspect = await api("POST", "/resources/skills/inspect-local", { path: folder });
    expect(inspect.statusCode).toBe(200);
    const preview = (await inspect.json()) as { skills: { name: string; extraFileCount: number }[] };
    ...
    expect(Buffer.from(readFileSync(skillFile("iconic", path.join("assets", "icon.png"))))).toEqual(png);
